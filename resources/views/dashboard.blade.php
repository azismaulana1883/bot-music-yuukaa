...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-extrabold tracking-tight text-white">Server Discord Kamu</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($guilds as $guild)
            <div class="glass-card rounded-2xl p-5 group flex flex-col justify-between h-48 relative overflow-hidden">
                ...
                <div class="flex items-start gap-4">
                    ...
                    @if($guild['icon'])
                        <img src="https://cdn.discordapp.com/icons/{{ $guild['id'] }}/{{ $guild['icon'] }}.png" 
                             alt="{{ $guild['name'] }}" 
                             class="w-14 h-14 rounded-xl shadow-md border border-slate-900 group-hover:scale-105 transition-transform duration-300">
                    @else
                        <div class="w-14 h-14 rounded-xl bg-gradient-to-tr from-slate-900 to-slate-950 border border-slate-800 flex items-center justify-center font-bold text-lg text-slate-300 group-hover:scale-105 transition-transform duration-300">
                            {{ Str::upper(Str::substr($guild['name'], 0, 2)) }}
                        </div>
                    @endif

                    <div class="flex-1 min-w-0">
                        <h2 class="text-lg font-bold text-white truncate group-hover:text-violet-400 transition-colors" title="{{ $guild['name'] }}">
                            {{ $guild['name'] }}
                        </h2>
                        ...
                        <div class="mt-1.5">
                            @if($guild['has_bot'])
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-violet-500/10 text-violet-300 border border-violet-500/20">
                                    <span class="w-1.5 h-1.5 rounded-full bg-violet-400"></span>
                                    Terhubung
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-900 text-slate-400 border border-slate-800/60">
